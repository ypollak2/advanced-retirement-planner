...
const ResultsPanel = ({ 
    ...
    // Check if we have meaningful data for readiness score
    ...
    return React.createElement('div', { className: "space-y-6" }, [
        // Retirement Readiness Score (only show when we have meaningful data)
        ReadinessScore && hasMeaningfulDataForScore && React.createElement(ReadinessScore, {
            key: 'readiness-score',
            currentAge: inputs?.currentAge || effectiveResults.currentAge || 0, // Use 0 instead of hardcoded 30
            retirementAge: inputs?.retirementAge || effectiveResults.retirementAge || 0, // Use 0 instead of hardcoded 67
            currentSavings: inputs?.currentSavings || 0, // Remove hardcoded 50000 default
            monthlyContribution: (inputs?.monthlyContribution || 0) + (inputs?.trainingFundMonthly || 0),
            targetRetirementIncome: inputs?.currentMonthlyExpenses || effectiveResults.monthlyIncome || 0, // Remove hardcoded 15000 default
