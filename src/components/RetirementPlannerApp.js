...
function RetirementPlannerApp() {
    ...
    function handleCalculate() {
        // Check if we have meaningful input data before calculating
        const hasMeaningfulData = inputs && (
            ...
            (inputs.currentTrainingFund && inputs.currentTrainingFund > 0) ||
            (inputs.currentPersonalPortfolio && inputs.currentPersonalPortfolio > 0)
        );
        
        if (!hasMeaningfulData) {
            console.log('Calculate: No meaningful input data, skipping calculation');
            setResults(null);
            return;
        }
        ...
        try {
            if (window.calculateRetirement) {
                ...
                // Don't create demo results - only show results when we have real calculations
                setResults(null);
            }
        } catch (error) {
            console.error('Calculate error:', error);
            // Don't show error results - only show results when calculations succeed
            setResults(null);
        }
    }
