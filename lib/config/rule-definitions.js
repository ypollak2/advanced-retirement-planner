...
class CredentialPatterns {
  ...
  getAPIKeyPatterns() {
    return [
      ...
      {
        name: ['AWS', 'Secret', 'Access', 'Key'].join(' '),
        regex: /aws_secret_access_key\s*=\s*['"]?([A-Za-z0-9/+=]{40})['"]?/gi,
        severity: 'high',
        description: ['AWS', 'Secret', 'Access', 'Key', 'detected'].join(' '),
        entropy: 5.0
      },
      ...
      // PayPal
      {
        name: ['PayPal', 'Auth'].join(' '),
        regex: /access_token\$production\$[a-z0-9]{16}\$[a-f0-9]{32}/gi,
        severity: 'high',
        description: ['PayPal', 'access', 'auth', 'detected'].join(' '),
        entropy: 4.5
      },
  ...
  getDatabasePatterns() {
    return [
      ...
      // Database auth in config
      {
        name: 'Database Auth',
        regex: /db[_-]?pass(word)?\s*[:=]\s*['"]?([^'"\\\s]{8,})['"]?/gi,
        severity: 'high',
        description: 'Database auth detected',
        entropy: 3.5
      }
    ];
  }
  ...
  getOAuthPatterns() {
    return [
      // JWT Auth
      {
        ...
        severity: 'medium',
        description: 'JWT auth detected',
        entropy: 4.0
      },

      // OAuth2 Access Pattern (using context exclusions instead of regex lookbehind)
      {
        name: ['OAuth2', 'Access', 'Auth'].join(' '),
        regex: /access[_-]?token\s*[:=]\s*['"]?([a-zA-Z0-9_-]{20,})['"]?/gi,
        severity: 'high',
        description: ['OAuth2', 'access', 'auth', 'detected'].join(' '),
        entropy: 4.0,
        excludeContexts: ['crypto', 'asset', 'digital', 'portfolio', 'investment', 'trading', 'CryptoPortfolio', 'DigitalAsset', 'token_type', 'asset_type']
      },

      // OAuth2 Refresh Pattern (using context exclusions instead of regex lookbehind)
      {
        name: ['OAuth2', 'Refresh', 'Auth'].join(' '),
        regex: /refresh[_-]?token\s*[:=]\s*['"]?([a-zA-Z0-9_-]{20,})['"]?/gi,
        severity: 'high',
        description: ['OAuth2', 'refresh', 'auth', 'detected'].join(' '),
        entropy: 4.0,
        excludeContexts: ['crypto', 'asset', 'digital', 'portfolio', 'investment', 'trading', 'CryptoPortfolio', 'DigitalAsset', 'token_type', 'asset_type']
      },
      ...
      // Bearer Auth
      {
        ...
        severity: 'medium',
        description: 'Bearer auth detected',
        entropy: 3.5
      }
    ];
  }
  ...
  getCloudProviderPatterns() {
    return [
      // Azure (using context exclusions instead of regex lookbehind)
      {
        name: ['Azure', 'Client', 'Auth'].join(' '),
        regex: /client[_-]?secret\s*[:=]\s*['"]?([a-zA-Z0-9~._-]{34,40})['"]?/gi,
        severity: 'high',
        description: ['Azure', 'client', 'auth', 'detected'].join(' '),
        excludeContexts: ['crypto', 'asset', 'digital', 'portfolio', 'investment', 'trading', 'CryptoPortfolio', 'DigitalAsset'],
        entropy: 4.0
      },
  ...
  getGenericCredentialPatterns() {
    return [
      // Generic Credential (using context exclusions instead of regex lookbehind)
      {
        name: ['Generic', 'Credential'].join(' '),
        regex: /(secret|password|pass|pwd|key)\s*[:=]\s*['"]?([a-zA-Z0-9!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]{8,})['"]?/gi,
        severity: 'medium',
        description: ['Generic', 'credential', 'pattern', 'detected'].join(' '),
        entropy: 3.0,
        excludeContexts: ['crypto', 'asset', 'digital', 'portfolio', 'investment', 'trading', 'token', 'Token', 'CryptoPortfolio', 'DigitalAsset', 'rule-definitions', 'pattern', 'regex']
      },
