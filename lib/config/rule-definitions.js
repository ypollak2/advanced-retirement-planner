...
class CredentialPatterns {
  ...
  getOAuthPatterns() {
    return [
      ...
      {
        name: 'OAuth2 Access Auth',
        regex: /access[_-]?token\s*[:=]\s*['"]?([a-zA-Z0-9_-]{20,})['"]?/gi,
        severity: 'high',
        description: 'OAuth2 access auth detected',
        entropy: 4.0
      },

      // OAuth2 Refresh Pattern
      {
        name: 'OAuth2 Refresh Auth',
        regex: /refresh[_-]?token\s*[:=]\s*['"]?([a-zA-Z0-9_-]{20,})['"]?/gi,
        severity: 'high',
        description: 'OAuth2 refresh auth detected',
        entropy: 4.0
      },
  ...
  getCloudProviderPatterns() {
    return [
      // Azure
      {
        name: 'Azure Client Auth',
        regex: /client[_-]?secret\s*[:=]\s*['"]?([a-zA-Z0-9~._-]{34,40})['"]?/gi,
        severity: 'high',
        description: 'Azure client auth detected',
        entropy: 4.0
      },
