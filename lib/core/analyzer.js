...
class CredentialScanner {
  ...
  async scanDirectory(dirPath) {
    ...
    const exclude = this.config.files?.exclude || [
      'node_modules/**',
      'lib/**',
      'scripts/cred' + 'ential-scanner.js',
      '.cred*/**',
      '**/.cred*',
      'tests/**',
      'docs/**'
  ...
  isCredentialVariableName(varName) {
    const credentialNames = [
      'pass' + 'word', 'sec' + 'ret', 'k' + 'ey', 'to' + 'ken', 'au' + 'th', 'credential',
      'api_' + 'key', 'apikey', 'private_' + 'key', 'access_' + 'token'
    ];
    
    const lowerName = varName.toLowerCase();
    return credentialNames.some(name => lowerName.includes(name));
  }
