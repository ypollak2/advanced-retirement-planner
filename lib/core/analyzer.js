/**
 * Core Secret Analyzer with AST-based Semantic Analysis
 * 
...
const SecretPatterns = require('../config/rule-definitions');
...
class SecretScanner {
  constructor(options = {}) {
    ...
    this.patterns = SecretPatterns.getPatterns();
    this.config = {};
    this.stats = {
      filesScanned: 0,
      findingsCount: 0,
  ...
  async scanDirectory(dirPath) {
    ...
    const exclude = this.config.files?.exclude || ['node_modules/**'];
      ...
      traverse(ast, {
        VariableDeclarator: (nodePath) => {
          ...
          if (node.id.name && node.init) {
            ...
            if (this.isSecretVariableName(varName)) {
              findings.push(this.createFinding({
                ...
                filePath,
                severity: 'medium',
                description: `Potential secret variable: ${varName}`
              }));
            }
          }
        },
  ...
  isSecretVariableName(varName) {
    ...
    return secretNames.some(name => lowerName.includes(name));
  }
  ...
  generateConsoleReport(findings) {
    ...
    let report = `🔍 Secret Scan Results\n${'='.repeat(50)}\n\n`;
