...
class CredentialScanner {
   ...
   * Excludes cryptocurrency/asset-related variables
   */
  isCredentialVariableName(varName) {
    ...
    // Cryptocurrency/asset-related terms that should be excluded
    const cryptoAssetTerms = [
      'crypto', 'asset', 'digital', 'btc', 'eth', 'coin', 'portfolio', 
      'investment', 'trading', 'token_type', 'asset_type', 'crypto_asset', 
      'digital_asset', 'tokenfield', 'assettoken', 'handletoken'
    ];
    
    const lowerName = varName.toLowerCase();
    
    // If it contains crypto/asset terms, don't flag as credential
    if (cryptoAssetTerms.some(term => lowerName.includes(term))) {
      return false;
    }
